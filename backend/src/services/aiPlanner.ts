...
const fallbackPlan = (input: string): TaskPlan => ({
  steps: [
...
export async function generateTaskPlan(input: string): Promise<TaskPlan> {
  if (!client) {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as OpenAIPlanResponse;
